<div class="product-card flex flex-col md:flex-row rounded-3xl bg-card">
    ...
    <div class="grow flex flex-col py-8 px-6">
        <h3 class="text-sm lg:text-md font-bold"><a href="{{route('product', $item->slug)}}"
        ...
        @if($item->json_properties)
            <ul class="space-y-1 mt-4 text-xxs">
                ...
                @foreach($item->json_properties as $property => $value)
                    <li class="flex justify-between "><strong>{{$property}}:</strong> {{$value}}</li>
                @endforeach
        ...
        <div class="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-6 mt-6">
            <div class="flex items-baseline gap-4">
                <div class="text-pink text-md xl:text-lg font-bold">{{$item->price}}</div>
                <div class=" text-sm xl:text-md font-semibold line-through">{{$item->old_price}}</div>
            </div>
            <div class="flex flex-wrap items-center gap-4">
                ...
                @auth
                    @if(favorite()->check($item->id))
                        <form action="{{route('favorite.delete', $item)}}" method="post">
                            @csrf
                            @method('delete')
                            <button class="w-[56px] !h-[56px] !px-0 btn btn-purple" title="Удалить из избранного">
                    ...
                    @else
                        <form action="{{route('favorite.add', $item)}}" method="post">
                            @csrf
                            <button class="w-[56px] !h-[56px] !px-0 btn btn-purple" title="В избранное">
                                <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="currentColor"
                                     viewBox="0 0 52 52">
                                    <path
